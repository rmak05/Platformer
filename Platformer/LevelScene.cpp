...
void LevelScene::do_action(Action _action) {
	...
	switch (_action.get_name()) {
		case ActionName::jump: {
			...
			if (_action.get_type() == ActionType::start) {
				...
				if(_cmotion.velocity.y == 0.0f){
					...
					player_ptr->set_component<CShape>().shape = player_config.animation_still;
				}
			}
		...
		case ActionName::left: {
			auto& _cmotion  = player_ptr->get_component<CMotion>();
			auto& _cshape	= player_ptr->get_component<CShape>();
			auto _scale		= _cshape.shape.get_scale();

			if		(_action.get_type() == ActionType::start) {
				player_ptr->get_component<CInput>().left = true;
				_cmotion.velocity.x += (-player_config.run_velocity);
				_cshape.shape.set_scale(std::fabsf(_scale.x) * (-1), _scale.y);
			}
			...
			if		(_cmotion.velocity == sf::Vector2f(0.0f, 0.0f)) player_ptr->set_component<CShape>().shape = player_config.animation_still;
			else if (_cmotion.velocity.y == 0.0f) player_ptr->set_component<CShape>().shape = player_config.animation_moving;
		...
		case ActionName::right: {
			...
			if		(_action.get_type() == ActionType::start) {
				...
				_cmotion.velocity.x += (player_config.run_velocity);
				_cshape.shape.set_scale(std::fabsf(_scale.x), _scale.y);
			}
			else if (_action.get_type() == ActionType::end) {
...
void LevelScene::spawn_player() {
	...
	player_ptr->set_component<CTransform>(grid_to_mid_coord(sf::Vector2f(player_config.start_pos_x, player_config.start_pos_y), player_config.animation_still));
	player_ptr->set_component<CBoundingBox>(player_config.animation_still.get_scaled_size());
...
void LevelScene::spawn_bullet() {
	...
	_bullet->set_component<CTransform>(player_ptr->get_component<CTransform>().curr_position + sf::Vector2f(32.0f, -8.0f));
	...
	_explosion->set_component<CTransform>(player_ptr->get_component<CTransform>().curr_position + sf::Vector2f(44.0f, -8.0f));
}
